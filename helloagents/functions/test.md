...
规则:
  ...
  6. 只要本轮新增或修改了测试文件，就必须完成一次子代理侧 test_audit；没有该结果不得输出完成摘要
```
...
规则:
  ...
  无稳定可写子代理能力时:
    非测试文件触发源 → 主代理按同一协议降级执行
    新增/修改测试文件触发源 → 视为阻断失败，不得 direct 放行
  不得把 test_audit_cycle 简化成“再跑一次测试”
  只要 `test_files` 非空，就必须获得 `via != direct` 的 test_audit 结果，否则视为阻断失败
```
