...
原生子代理映射（角色→类型映射，调用语法详见各 CLI 调用协议）:
  ...
  测试审计（test_audit_cycle / test_auditor）→ Codex: spawn_agent(agent_type="worker") | Claude: Agent(subagent_type="general-purpose") | OpenCode: general | Gemini: generalist | Qwen: general-purpose
...
强制调用规则（标注"强制"的必须调用，标注"跳过"的可跳过，子代理编排均遵循自动编排原则）:
  ...
  DEVELOP:
    子代理（按编排五步法选择类型）— ≥2个可独立并行的任务项 自动编排（步骤6，按 DAG 或主代理判断依赖后并行）| 新增测试用例时自动编排（步骤8A）| 触发 test_audit_cycle 时强制调度测试审计子代理（步骤8B）| 仅1个任务项 主代理直接执行
      追加硬约束: 只要本轮新增或修改了测试文件，test_audit_cycle 必须产出一份子代理侧结果；主代理 direct 审计不得用于放行完成态
...
适用场景与编排策略（步骤2c 的默认原生类型）:
  ...
  质量检查（审查/验证/测试）:
    ...
    → 默认类型: general-purpose / worker
  测试审计（post-test audit）:
    → 针对本轮测试资产和测试结果执行二次审计，不得复用只读 reviewer
    → 默认类型: general-purpose / worker
    → 审计维度固定包含覆盖率充分性、断言有效性、测试时效性
    → 允许在职责范围内直接修复测试或少量配套代码，并回传 blind_spots（思维局限/盲区）
  运行入口恢复（ide_restart_recovery）:
    → 主代理主导的能力阶梯，不要求专属子代理
    → 可借助可用 Skill/MCP 辅助：IDE 原生运行配置工具 > 控制电脑类 MCP/skills > 手动重启
...
prompt 构造模板:
  ...
  职责边界按任务类型适配:
    ...
    测试审计 → "你负责: 执行 test_audit_cycle。审计范围: {测试文件列表} 与 {被测代码范围}。检查覆盖率充分性、断言有效性、测试时效性；必要时直接修复测试或小范围配套代码，并总结 blind_spots。"
  ...
  标准返回格式（代码实现/测试编写类子代理强制，其他类型按需）:
    ...
    test_audit:
      coverage_assessment: "覆盖率充分性结论"
      correctness_assessment: "断言与行为匹配度结论"
      staleness_assessment: "测试时效性结论"
      blind_spots: ["思维局限/盲区"]
      repairs_applied: ["已执行修复"]
      retest_commands: ["建议或已执行的回归命令"]
      parent_guidance: ["返回给主代理的建议"]
