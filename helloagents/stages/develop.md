...
调试/修复任务（强制）:
  加载并应用 [→ rules/debugging.md]
  编码前先确认:
    - 待改位置是否属于 First Bad Divergence 的拥有者
    - 修复是否优先恢复稳定源/正确优先级，而不是在症状层打补丁
    - working path 是否仍保持不变
  若存在“稳定上游产物 vs 本地派生产物”竞争关系:
    优先复用已验证稳定的上游产物
    仅在稳定产物缺失或不可用时保留本地重算/重渲染
...
编程任务:
  工具探测（步骤开始时，≤3秒）:
    ...
    → 识别可用的验证能力（lint、类型检查、单元测试、覆盖率、E2E、构建、视觉验证、IDE运行配置、控制电脑类自动化等）
    ...
    视觉验证能力探测（UI 任务时必须执行）:
      ...
      探测结果供步骤9使用
    运行入口恢复能力探测（有独立运行入口的交付物时执行）:
      IDE原生级: 会话可直接执行 JetBrains 系 IDE 运行配置（如 IDEA/PyCharm/WebStorm 的运行配置执行工具）
      控制电脑级: 会话可控制桌面窗口/截图/快捷键/点击（Control MCP、桌面自动化 Skill 等）
      手动级: 以上均无 → 记录为“需用户手动重启项目”
      注: JetBrains 系 IDE 经验统一覆盖 IntelliJ IDEA / PyCharm / WebStorm / 其他同类 IDE
  ...
  8B. 测试审计闭环（test_audit_cycle，CRITICAL）:
    触发条件（满足任一即强制）:
      本轮新增或修改了测试文件
      本步骤中由主代理或测试子代理新增了测试用例
      本轮改动为核心逻辑，但现有测试覆盖不足或覆盖率工具显示存在明显空洞
    目标:
      审计覆盖率充分性
      审计断言是否真实验证行为（避免伪通过/只测表层）
      审计测试是否因实现演进而过时
      必要时直接修复测试或小范围配套代码
      把思维局限/盲区回传主代理，纳入验收报告
    主代理必须补充的上下文包:
      本轮改动文件列表
      本轮新增/修改的测试文件列表
      被测代码 ↔ 测试文件映射
      ...
      主代理重点关注的风险点
    子代理调用:
      按编排五步法调度一个“测试审计”工作单元 [→ G10 调用通道]
      审计角色语义: test_auditor（可写），不得复用只读 reviewer
      Claude Code: Agent(subagent_type="general-purpose") + test_auditor 约束
    ...
    结果要求:
      ...
      返回 repairs_applied / retest_commands / parent_guidance
    审计失败处理:
      发现可直接修复的问题 → 子代理直接修复 → 进入 8C
      发现高风险但无法安全自动修复的问题 → ⛔ 阻断性 → 输出: 警告 → ⛔ END_TURN
      无覆盖率工具 → 允许启发式评估，但必须显式标注“不含真实覆盖率数据”

  8C. 修复与回归:
    8B 有 repairs_applied → 必须重新运行受影响的测试命令
    8B 发现新增盲区但未修复 → 主代理或后续子代理补充修复，再回归
    调试/修复任务追加:
      回归用例必须覆盖“异常触发条件”和“正常对照条件”两边
      若问题来自 Source/Derived/Persisted/Presentation 分叉:
        断言中至少覆盖一个 working path 和一个 failing path 的边界
    回归测试仍失败:
      ⛔ 阻断性(核心功能): 输出: 警告 → ⛔ END_TURN
      ⚠️ 警告性(非核心路径): 记录继续
    验收报告必须包含:
      已执行的测试审计结论
      新增或修复的测试摘要
...
**DO NOT:** 将测试审计塞进 reviewer，只读审查与可修复审计必须分离
**DO NOT:** 修复无关错误或失效测试，向没有格式化器的代码库添加格式化器
```

### 步骤9: 运行入口恢复与交付验收（⛔ 有独立运行入口的交付物必须实际启动验证，不可跳过）

```yaml
原则:
  步骤9A 先完成运行入口恢复（ide_restart_recovery）
  步骤9B 再完成功能/需求/体验验收
  测试通过 ≠ 用户可用；运行入口未恢复 ≠ 完成交付

9A. 运行入口恢复（ide_restart_recovery，CRITICAL）:
  适用:
    有独立运行入口的交付物
    代码更新后需要重新运行或重启主入口配置的场景
  执行顺序（按能力阶梯，命中即停）:
    IDE原生级:
      会话存在 JetBrains 系 IDE 运行配置执行能力（如 IDEA/PyCharm/WebStorm 运行配置工具）
      → 直接重新执行/重启受影响的主运行配置
    控制电脑级:
      无直接重启能力，但存在控制电脑类 MCP/skills
      → 聚焦 JetBrains 系 IDE 窗口
      → 必要时截屏/截窗口确认状态
      → 使用快捷键或点击执行“停止后重新运行”或等效动作
      → 必要时再次截图确认重启已发生
    手动级:
      以上均无
      → 在验收报告中明确要求用户手动重启项目
      → 给出最小可执行步骤（如“回到 IDE，重新运行主配置”）
  规则:
    JetBrains 经验统一覆盖 IntelliJ IDEA / PyCharm / WebStorm / 其他同类 IDE
    不得假设固定 UI 布局、固定快捷键或固定主题
    必须先探测能力，再决定执行路径
    自动化重启失败且无后续降级路径 → ⚠️ 警告性，要求用户手动重启
...
9B. 交付验收:
  ...
  体验验收（complex 全维度 | moderate + 交付物含 UI/API/CLI 时核心维度）:
    UI/前端（对照 proposal.md "成果设计" 节验证）:
      ...
      验证方式: 按步骤8探测的视觉验证能力级别执行对应验证策略
    API/后端（complex 全部，moderate 仅标★项）:
      ★响应格式一致性 + 错误码规范 + ★核心边界输入处理
    CLI/工具（complex 全部，moderate 仅标★项）:
      ★帮助信息完整 + ★错误提示清晰 + 参数边界处理
    文档:
      受众适配度 + 示例覆盖 + 术语一致性
    发现问题 → ⚠️ 警告性，记录到验收报告
  ...
  验收透明度（moderate + complex，嵌入完成输出）:
    ...
    固定项（始终包含）:
      ...
      "测试审计盲区: 对照 blind_spots，确认是否仍有需人工补测的区域"
    DELEGATED 模式追加:
