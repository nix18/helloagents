...
**Post-Test Audit Cycle** (cross-CLI): after tests are written or updated, DEVELOP now requires a second-pass test audit before final acceptance. The parent agent must provide the changed files, changed tests, executed test results, available coverage signals, and focus risks; a writable sub-agent then audits coverage sufficiency, assertion correctness, and test staleness, may directly repair tests or small supporting code, and returns blind spots plus retest guidance to the parent agent. The runtime now treats this as a unified post-test pipeline, so DEVELOP and `~test` consume the same audit/retest flow instead of maintaining separate logic.
