...
## **Important: How to install this branch：**
...
**Generalized debugging reasoning** (cross-CLI): for bugfix, debug, and regression tasks, HelloAGENTS now requires explicit separation of Source / Derived / Persisted / Transport / Presentation representations, plus identification of the first divergence between the working path and the failing path. Cases such as “runtime works but history/replay breaks”, “raw field is correct but derived artifact is wrong”, or “half the output is correct and half is broken” should be analyzed by tracing the first bad divergence and fixing the owning layer, priority rule, or normalization step — not by patching the last visible symptom.

**Post-Test Audit Cycle** (cross-CLI): after tests are written or updated, DEVELOP now requires a second-pass test audit before final acceptance. The parent agent must provide the changed files, changed tests, executed test results, available coverage signals, and focus risks; a writable sub-agent then audits coverage sufficiency, assertion correctness, and test staleness, may directly repair tests or small supporting code, and returns blind spots plus retest guidance to the parent agent. The runtime now treats this as a unified post-test pipeline, so DEVELOP and `~test` consume the same audit/retest flow instead of maintaining separate logic. This is now a hard gate: if the current turn added or modified test files, the workflow must obtain a sub-agent-side audit result before it can finish.
