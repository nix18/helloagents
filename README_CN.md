...
**测试后二次审计闭环**（跨 CLI）：当测试被新增或更新后，DEVELOP 阶段现在要求在最终验收前执行一次额外的测试审计。主代理必须补充本轮改动文件、测试改动、已执行测试结果、覆盖率信号和重点风险；随后由可写子代理审计覆盖率充分性、断言有效性和测试时效性，必要时直接修复测试或少量配套代码，并把 blind spots（思维局限/盲区）和回归建议返回给主代理。
