...
> [!IMPORTANT]
> **需要旧版本的 HelloAGENTS？** 旧版代码库已迁移至独立归档仓库：[helloagents-archive](https://github.com/nix18/helloagents-archive)
...
**更新：** `uv tool install --from git+https://github.com/nix18/helloagents helloagents --force`

**卸载：** `uv tool uninstall helloagents`

**切换分支：** `uv tool install --from git+https://github.com/nix18/helloagents@beta helloagents --force`
...
**更新：** `pip install --upgrade git+https://github.com/nix18/helloagents.git`

**卸载：** `pip uninstall helloagents`

**切换分支：** `pip install --upgrade git+https://github.com/nix18/helloagents.git@beta`
...
**泛化问题定位推理**（跨 CLI）：对于 bugfix/debug/回归排障任务，HelloAGENTS 现在要求显式区分 Source/Derived/Persisted/Transport/Presentation 五类表示层，并先找出正常链路与异常链路的第一处分叉（First Bad Divergence），再决定修复点。像“运行时正常、历史回放异常”“原始字段正常、派生产物错误”“部分内容正确、部分内容错误”这类问题，不再只靠临场发挥，而是按统一规则比较 working path 与 failing path，优先修正分叉拥有者、错误优先级或错误归一化逻辑，而不是在症状层打补丁。

**测试后二次审计闭环**（跨 CLI）：当测试被新增或更新后，DEVELOP 阶段现在要求在最终验收前执行一次额外的测试审计。主代理必须补充本轮改动文件、测试改动、已执行测试结果、覆盖率信号和重点风险；随后由可写子代理审计覆盖率充分性、断言有效性和测试时效性，必要时直接修复测试或少量配套代码，并把 blind spots（思维局限/盲区）和回归建议返回给主代理。当前运行时已把这条能力收口为统一的 post-test pipeline，`DEVELOP` 与 `~test` 共享同一套审计/回归逻辑。

**运行入口恢复验证**（跨 CLI）：在最终验收前，HelloAGENTS 会优先尝试恢复已更新交付物的运行入口。对于 JetBrains 系 IDE（IntelliJ IDEA、PyCharm、WebStorm 等），优先使用 IDE 原生运行配置执行能力；若无直接能力，则降级为控制电脑类 MCP/skills（聚焦窗口、截屏/截窗、快捷键/点击重启）；若仍不可用，则明确要求用户手动重启项目。
...
**解决：** 先安装包：`pip install git+https://github.com/nix18/helloagents.git` 或使用 UV/npx 方式
...
- 在 [GitHub Issues](https://github.com/nix18/helloagents/issues) 报告问题或请求功能
...
只要本轮新增或修改了测试文件，DEVELOP 和 `~test` 就必须执行一次子代理侧 `test_audit_cycle`，拿到可写子代理返回的审计结果后，才允许进入回归、验收和完成态。主代理 direct 审计不再能放行这类场景。
